...
public class Connection {
    ...
    public void close() {
    	log.error("##########关闭连接：{}：{}",new Object[]{host,port});
        try {
     ...
     * 
     * TODO 
     * 1，发送消息超时，连接暂时移开，异步检测连接是否可用，不可用关闭重练
     * 
    ...
    public NSQFrame commandAndWait(NSQCommand command) throws TimeoutException {
        ...
        try {
            ...
            if (!this.requests.offer(command, 15, TimeUnit.SECONDS)) {
            	...
            	//还没发
                throw new TimeoutException("command: " + command + " timedout", null);
            }
            ...
            throw new TimeoutException("command: " + command + " timedout", x);
        } finally{
