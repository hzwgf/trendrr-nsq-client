...
import com.trendrr.nsq.exceptions.NoSpaceLeftException;
import com.trendrr.nsq.exceptions.TimeoutException;
...
public class NSQProducer extends AbstractNSQClient {
     ...
     */
    public void produceMulti(String topic, List<byte[]> message) throws DisconnectedException, BadTopicException, BadMessageException, NoConnectionsException, NoSpaceLeftException, TimeoutException{
     ...
     * @throws TimeoutException 
     */
    public void produce(String topic, byte[] message) throws DisconnectedException, BadTopicException, BadMessageException, NoConnectionsException, NoSpaceLeftException, TimeoutException{
        if(state.get()==0){
        	throw new DisconnectedException("客户端没有启动",null);
        } else if(state.get()==2){
        	throw new DisconnectedException("客户端已关闭",null);
        }
    	Connection c = this.getConn();
    	if(c==null){
    		throw new DisconnectedException("没有连接了",null);
    	}
        ...
        if (frame instanceof ErrorFrame) {
            ...
            if (err.startsWith("E_BAD_TOPIC")) {
                throw new BadTopicException(err);
            }else if (err.startsWith("E_BAD_MESSAGE")) {
                throw new BadMessageException(err);
            }else if(err.endsWith("no space left on device")){
            	 throw new NoSpaceLeftException("no space left on device",null);
            }
            return;
        }
        //disconnect
